    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#020617] py-24 px-4 font-sans transition-colors">
            <div className="max-w-6xl mx-auto">
                ...
                <header className="flex flex-col items-center  font-poppins text-center mb-12">
                    <div className="p-3 bg-emerald-500/10  font-poppins text-emerald-500 rounded-2xl mb-4 border border-emerald-500/20">
                        <LuCalculator size={28} />
                    </div>
                    <h1 className=" font-poppins text-3xl font-poppins text-4xl font-black font-poppins font-poppins text-black dark:text-white capitalize  ">
                        Medical <span className=" font-poppins text-emerald-500">Cost Insurance</span> Prediction
                    </h1>
                </header>
                    ...
                    <form onSubmit={handleSubmit} className="lg:col-span-7 grid grid-cols-1 md:grid-cols-2 gap-5 bg-white dark:bg-slate-900 p-8 md:p-10 rounded-[2.5rem] shadow-xl border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="space-y-1">
                            <div className="flex justify-between items-center px-1">
                                <label className=" font-poppins text-[10px] font-black  font-poppins text-slate-400 uppercase  ">BMI</label>
                                <button type="button" onClick={() => setShowBmiCalc(true)} className=" font-poppins text-[9px] font-black  font-poppins text-emerald-500 uppercase hover: font-poppins text-emerald-600 underline underline-offset-2">Calculate BMI</button>
                            </div>
                            <motion.input
                        ...
                        {error && <p className="md:col-span-2  font-poppins text-rose-500  font-poppins text-[10px] font-black uppercase  font-poppins text-center  ">{error}</p>}

                        <button disabled={loading} className="md:col-span-2 mt-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-400  font-poppins text-white font-black py-5 rounded-2xl transition-all flex items-center justify-center gap-3 uppercase    font-poppins text-xs shadow-lg shadow-emerald-500/20 active:scale-95">
                        ...
                        <AnimatePresence mode="wait">
                            {result && !loading ? (
                                <motion.div
                                    ...
                                    className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/50 p-8 md:p-10 rounded-[3rem]  font-poppins text-white shadow-2xl relative overflow-hidden"
                                >
                                    {/* Header info */}
                                    ...
                                    <div className="bg-emerald-400/50 backdrop-blur-md rounded-[2rem] p-7 border border-white/20 mb-6 shadow-inner">
                                        <div className="flex justify-between items-center mb-1">
                                            <p className=" font-poppins text-[10px] font-bold uppercase   opacity-80">Trusted Prediction (Regressor)</p>
                                            <span className="bg-emerald-400/20  font-poppins text-[8px] font-black px-2 py-0.5 rounded-full border border-emerald-400/30">PRIMARY</span>
                                        </div>
                                        <p className=" font-poppins text-5xl font-black ">
                                            ₹{result.cost.regressor.toLocaleString('en-IN')}
                                            <span className=" font-poppins text-lg opacity-60 font-medium ml-1">/ month</span>
                                        </p>
                                    </div>

                                    {/* Comparative Models (Alpha & Beta) */}
                                    <div className="grid grid-cols-2 gap-4 mb-6">
                                        <div className="bg-emerald-400/50 backdrop-blur-sm rounded-2xl p-5 border border-white/10">
                                            <p className=" font-poppins text-[8px] font-black uppercase   opacity-50 mb-1">Alpha Model (LR)</p>
                                            <p className=" font-poppins text-xl font-bold">₹{result.cost.model.toLocaleString('en-IN')}</p>
                                        </div>
                                        <div className="bg-purple-400/50 backdrop-blur-sm rounded-2xl p-5 border border-white/10">
                                            <p className=" font-poppins text-[8px] font-black uppercase   opacity-50 mb-1">Beta Model (NN)</p>
                                            <p className=" font-poppins text-xl font-bold">{result.cost.nn ? "₹" + result.cost.nn.toLocaleString('en-IN') : "Under Maintenance"}</p>
                                        </div>
                                    </div>
                                ...
                                <div className="h-full min-h-[450px] border-4 border-dashed border-slate-200 dark:border-slate-800 rounded-[3rem] flex flex-col items-center justify-center p-12  font-poppins text-center  font-poppins text-slate-300">
                                    <div className="relative mb-6">
                                        <LuInfo size={48} className="opacity-10" />
                                        <motion.div
                                            animate={{ rotate: 360 }}
                                            transition={{ duration: 8, repeat: Infinity, ease: "linear" }}
                                            className="absolute inset-0 border-2 border-dashed border-emerald-500/20 rounded-full scale-150"
                                        />
                                    </div>
                                    <p className=" font-poppins text-[10px] font-black uppercase ">Neural Inference Pending</p>
                                    <p className=" font-poppins text-[9px] mt-2 opacity-40 max-w-[180px]">Fill in patient metrics to begin multi-model cross-validation.</p>
                                </div>
                            )}
                        </AnimatePresence>
                    </div>
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ scale: 0.9, y: 20, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.9, y: 20, opacity: 0 }} className="relative bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl border border-slate-200 dark:border-slate-800">
                            ...
                            <div className="space-y-5">
                                ...
                                <button onClick={handleApplyBmi} disabled={bmiApplying} className="w-full bg-slate-900 dark:bg-emerald-600  font-poppins text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 uppercase    font-poppins text-[10px] transition-all overflow-hidden relative">
                                    {bmiApplying ? (
                                ...
                                <footer className="pt-4 flex gap-2  font-poppins text-slate-400 border-t border-slate-100 dark:border-slate-800">
                                    <LuInfo size={12} className="shrink-0 mt-0.5" />
                                    <p className=" font-poppins text-[8px] font-medium leading-relaxed italic uppercase  ">
            ...
            <AnimatePresence>
                {result && !loading && (
                    ...
                    >
                        ...
                        <InsuranceMarket aiPredictedMonthly={result?.cost?.regressor || 1000} />
                    </motion.div>
                )}
            </AnimatePresence>
